...
Back in 2024, we discovered that a major telecom company had unintentionally exposed sensitive customer data through archived versions of its website. Using the Wayback Machine, we were able to access audio recordings and chat histories belonging to thousands of customers—data that should never have been publicly accessible.

In this blog, we’ll explain how we discovered the exposure, what information was accessible, why archived content can become a serious security risk, and how organizations can prevent similar incidents.
...
The Wayback Machine is an internet archive that periodically crawls and stores snapshots of publicly accessible websites, allowing users to view how pages looked in the past. However, if sensitive files, URLs, or endpoints are publicly accessible when a crawler visits them, they may also be archived. This can unintentionally preserve sensitive information long after it has been removed from the live website.


## What Was Reported

We reported finding a large number of audio recordings while examining historical information associated with `api.telecompany.com`.
